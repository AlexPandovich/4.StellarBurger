  ...
  reducers: {
    addIngredient: {
      reducer: (state, action) => {
        addToIngredientsCountMap(state.ingredientsCount, action.payload);

        if (action.payload.type === "bun") {
          if (state.bun) {
            deleteFromIngredientsCountMap(state.ingredientsCount, state.bun);
          }
    ...
    removeIngredient: (state, action) => {
      deleteFromIngredientsCountMap(state.ingredientsCount, action.payload);
      if (action.payload.type === "bun") {
        state.bun = null;
      } else {
